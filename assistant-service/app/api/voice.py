...
async def voice_input(websocket: WebSocket, settings: Settings = Depends(settings_dependency), stt_provider: SpeechToTextProvider = Depends(speech_to_text_provider_dependency)) -> None:
    token = websocket.headers.get("x-internal-gateway-token")
    if not token:
        protocols = websocket.headers.get("sec-websocket-protocol", "")
        token = next((part.strip() for part in protocols.split(",") if part.strip() and part.strip() != "gateway-token"), "")
    ...
    try:
        ...
        while True:
            ...
            if "bytes" in message and message["bytes"] is not None:
                if time.monotonic() - started_at > settings.assistant_max_audio_duration_seconds:
                    await stt_session.cancel()
                    await _send(websocket, _safe_error(request_id, "AUDIO_DURATION_EXCEEDED", "Audio request duration exceeded."))
                    return
                chunk = message["bytes"]
